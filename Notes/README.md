# 🎯 考研笔记总览

> 📍 `Ctrl+K` 打开侧边栏导航 · `Ctrl+F` 全文搜索

---

## 📐 高等数学

| 章节 | 状态 |
|:---|:---:|
| [题型分类总览](Math/calculus_overview.md) | ✅ 完成 |
| [泰勒展开与等价无穷小](Math/calculus_taylor.md) | ✅ 完成 |
| [常用不等式](Math/calculus_inequality.md) | ✅ 完成 |
| [求导公式大全](Math/calculus_derivative.md) | ✅ 完成 |
| [洛必达法则失效专题](Math/calculus_lhopital.md) | ✅ 完成 |
| [极限例题精讲](Math/calculus_limit.md) | ✅ 完成 |
| [连续与间断点](Math/calculus_continuous.md) | ✅ 完成 |

## 📗 英语

| 章节 | 状态 |
|:---|:---:|
| [单词本](English/Words.md) | 🔄 进行中 |
| [长难句](English/Long_sentence.md) | 🔄 进行中 |
| [阅读精析](English/Reading.md) | ✅ 已创建 |
| [作文练习](English/Composition.md) | ✅ 已创建 |

## 📘 政治

| 章节 | 状态 |
|:---|:---:|
| [政治笔记](Politics/politics.md) | ⏳ 待开始 |

## 💻 408 专业课

| 章节 | 状态 |
|:---|:---:|
| [408 笔记](408/Major.md) | ⏳ 待开始 |

---

## � 学习进度一览

| 科目 | 已完成 | 总章节 | 进度 |
|:---|:---:|:---:|:---|
| 📐 高等数学 | 7 | ~15 | ████████░░ 50% |
| 📗 英语 | — | — | 🔄 每日积累中 |
| 📘 政治 | 0 | — | ⏳ 未开始 |
| 💻 408 专业课 | 0 | — | ⏳ 未开始 |

---

## 📝 每日任务清单

<div id="checkin-area" style="text-align:center;margin:16px 0;">
  <span style="font-size:1.2em;" id="streak-text">🔥 加载中...</span><br>
  <button id="checkin-btn" onclick="doCheckin()" style="
    margin-top:8px;padding:10px 28px;font-size:1em;
    background:#42b983;color:#fff;border:none;border-radius:6px;cursor:pointer;
  ">📅 今日打卡</button>
  <p style="margin-top:4px;color:#888;font-size:0.85em;" id="checkin-hint"></p>
</div>

- [ ] 背单词 30 个（记录到 [生词本](English/Words.md)）
- [ ] 精读 1 篇阅读（记录到 [阅读精析](English/Reading.md)）
- [ ] 数学例题 2 道（按章节整理到对应笔记）
- [ ] 长难句分析 3 句

---

## 🛠 本站使用指南

| 功能 | 操作 |
|:---|:---|
| 🔍 全文搜索 | `Ctrl+F` 或顶部搜索框，支持中文全文检索 |
| 📂 侧边栏导航 | 左上角 **Menu** 按钮或 `Ctrl+K` |
| 📱 手机访问 | 响应式布局，支持横屏查看表格 |
| ✏️ 笔记更新 | VS Code 编辑 → Git Push → 自动部署 |

---

> 💡 **提示**：左上角 **Menu** 打开侧边栏，顶部搜索框可全文搜索所有笔记内容。
