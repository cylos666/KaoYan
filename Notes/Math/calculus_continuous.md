...
> **极限存在 ≠ 函数有定义，连续 = 极限 + 函数值统一**

- **极限存在**仅意味着在**去心邻域**内有定义。例如 $\lim\limits_{x\to 0} f(x) = a$，但 $f(0)$ 不一定等于 $a$（甚至可以不存在）。
- 若 $f(x)$ 在 $x=0$ 处**连续**，则 $f(0) = \lim\limits_{x\to 0} f(x) = a$，函数值与极限值统一。
- **在邻域内有定义 ≠ 极限存在**。例如 $\sin\frac{1}{x}$ 在 $x\to 0$ 时无限振荡，极限不存在。

> $$
> \text{连续} \iff \text{极限存在} \;\bold{且}\; \lim_{x\to x_0}f(x) = f(x_0)
> $$
...
> 遇到题目**先别跳进计算**，问自己：最终要证明/求出什么？逆向拆解。

### 范例：判断函数在可疑点是否连续

| 步骤 | 行动 | 说明 |
|:---:|:---|:---|
| **1** | 明确最终目标 | 判断 $f(x)$ 在 $x=x_0$ 处是否连续 |
| **2** | 写定义式 | $\lim\limits_{x\to x_0}f(x)=f(x_0)$ |
| **3** | 拆为两个子目标 | **A**：$f(x_0)$　**B**：$\lim f(x)$ |
| **4** | 判断是否需拆 B | 分段函数/含参极限 → 拆左极限 $x_0^-$ + 右极限 $x_0^+$ |
| **5** | 依次计算 | ① 求 $f(x)$ 完整表达式 → ② 代入得 $f(x_0)$ → ③ 算左右极限 |
| **6** | 比较结论 | $\lim f(x)$ 存在且 $=f(x_0)$ → ✅ 连续，否则 ❌ 不连续 |

> 💡 **心法**：目标 → 定义 → 拆解 → 计算 → 比较。每一步都知道"在算什么、为什么算"。
...
> 🔑 **核心矛盾**：零点定理（介值定理的特例）要求函数在**闭区间上连续**。如果区间内存在间断点，就不能直接套用——这是考研选择题/填空题的常见丢分点。

### 1. 零点定理回顾

若 $f(x)$ 在 $[a,b]$ 上**连续**，且 $f(a) \cdot f(b) < 0$，则 $\exists c \in (a,b)$ 使得 $f(c) = 0$。

> ⚠️ 关键词：**连续** + **闭区间** + **端点异号**

### 2. 有间断点时怎么办？——「分段讨论法」

当函数在 $[a,b]$ 上存在间断点时，以间断点为界拆分区间，在每个连续子区间上分别使用零点定理。

```
步骤：
  [a, b] 上有间断点 x₁, x₂, ...
       ↓
  拆分为连续子区间：[a, x₁], [x₁, x₂], ..., [xₙ, b]
       ↓
  在每个子区间上独立判断：
    1. 端点函数值是否异号？
    2. 函数在该子区间上是否连续？
...
| 间断点类型 | 对零点的影响 | 典型情况 |
|:---|:---|:---|
| **可去间断点** | 🟡 可能不影响 | 若补充定义后连续，零点仍可能存在 |
| **跳跃间断点** | 🔴 可能导致零点"跳过" | 端点异号，但函数跳过了 $x$ 轴，无零点 |
| **无穷间断点** | 🔴 分成两个独立区间 | 上下两侧分别判断，两侧可能都有或都没有零点 |
| **振荡间断点** | 🔴 需具体分析 | 如 $\sin\frac{1}{x}$ 在 $x\to0$ 附近振荡，零点分布复杂 |

### 4. 典型陷阱例题

> **例**：设 $f(x)=\begin{cases} -1, & x<0 \\ 1, & x \ge 0 \end{cases}$，在 $[-1,1]$ 上 $f(-1)=-1<0$，$f(1)=1>0$，端点异号，但 $f(x)$ **没有零点**——因为它只在 $x=0$ 处有一个跳跃间断点，函数值从 $-1$ 直接跳到 $1$，跳过了 $0$。

> ⚡ **教训**：**端点异号 + 连续 = 有零点**；端点异号 + 不连续 ≠ 一定有零点！

### 5. 考研实战模板

> **题目特征**：给出一个分段函数（通常含有 $[x]$、绝对值、$\operatorname{sgn}(x)$ 等），问在 $[a,b]$ 上零点的个数。

| 步骤 | 操作 |
|:---:|:---|
| ① | 找出所有间断点（分段点、无定义点） |
| ② | 以间断点为界拆分区间 |
| ③ | 在每个连续子区间上判断：端点是否异号？单调性如何？ |
| ④ | 汇总零点个数 |

> 📌 零点个数 = 各连续子区间零点数的和，每个子区间若端点异号且连续，则**至少**一个零点（严格单调则**恰好**一个）。

---

> 📎 **关联文件**：[极限例题精讲](/Math/calculus_limit) | [求导公式·极值与单调性](/Math/calculus_derivative) | [返回考研总览](/)
